...
class SymbolTable:
    ...
    def _set_predefined_symbols(self):
        for i in range(16):
            self.symbol_table.set_value("R"+i, i)
        i = 0
        for symbol in ["SCP", "LCL", "ARG", "THIS"]:
            self.symbol_table.set_value(symbol, i)
            i += 1

    def get_address(self, symbol):
        return self.map[symbol]

    def set_instruction_address(self, symbol, value):
        self.map[symbol] = value

    def generate_and_set_ram_address(self, symbol):
